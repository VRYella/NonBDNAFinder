...
Z-DNA extractor & annotator using Kadane algorithm for subarray detection.

This module implements Z-DNA detection using dinucleotide transition scoring
and a modified Kadane algorithm to find maximum scoring subarrays representing
potential Z-DNA forming regions.

Author: Adapted for user request with Kadane algorithm implementation.
Date: 2025-01-11

Requirements:
  - Python 3.8+
  - numpy, pandas, biopython, termcolor, attrs
  - Optional: hyperscan for accelerated pattern matching
"""
...
import time
import sys
from typing import Iterable, Iterator, Optional, Tuple, List, Dict, Any, Callable
...
# Import our new Z-DNA modules
from constants import Params
from zdna_calculator import ZDNACalculatorSeq

# -------------------------
# Submission form for processing
# -------------------------
...
# -------------------------
# Core extraction functions using Kadane algorithm
# -------------------------
def _extract(ID: str, sequence: str, params: Params) -> pd.DataFrame:
    """
    For a given sequence, calculates Z-DNA regions above threshold using Kadane algorithm.
    Returns them as a DataFrame.
    """
    zdna_calculator = ZDNACalculatorSeq(data=sequence, params=params)
    subarrays_detected = zdna_calculator.subarrays_above_threshold()
    scoring_array = zdna_calculator.scoring_array

    # subarrays_detected will have shape: [ (start, end, score, substring), ... ]
    if not subarrays_detected:
        # If no subarrays found, produce a row of NaNs for consistency
        total_score = float(np.sum(scoring_array)) if scoring_array.size > 0 else 0.0
        null_row = pd.DataFrame([{
            ...
            'Z-DNA Score': np.nan,
            'Sequence': np.nan,
            'totalSequenceScore': total_score
        }])
        return null_row[list(params.headers)]

    # Convert subarrays to DataFrame
    rows = []
    total_score = float(np.sum(scoring_array))
    for start, end, score, substring in subarrays_detected:
        rows.append({
            'Chromosome': ID,
            'Start': int(start),
            'End': int(end),
            'Z-DNA Score': float(score),
            'Sequence': substring,
            'totalSequenceScore': total_score
        })
    
    df = pd.DataFrame(rows)
    return df[list(params.headers)]
...
def _subextraction(submission_forms: List[ZDNASubmissionForm], params: Params) -> pd.DataFrame:
    ...
    if outputs:
        ...
        zdna_df = pd.DataFrame(columns=list(params.headers))
...
def extract_zdna_v2(fasta: str | os.PathLike[str], params: Params) -> pd.DataFrame:
    ...
    using parallel processing and Kadane algorithm. If `params.total_sequence_scoring` 
    is True, then skip subarray detection, returning a single transitions-based
    total score per sequence ID.
    """
    then = time.perf_counter()
    ...
    # and produce a single row per FASTA record with total score
    # ----------------------------------------------------------------------
...
def transform(path: Path | str, params: Params, gff_file: Path | None = None) -> pd.DataFrame:
    """
    Loads a FASTA, calculates Z-DNA features using Kadane algorithm, 
    optionally annotates with gene info (if gff_file is provided),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="""Given a FASTA file and parameters, calculates Z-DNA for each sequence present
                       using Kadane algorithm for subarray detection. Optionally, supply --gff_file to annotate hits with nearest genes."""
    )
