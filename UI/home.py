...
def _build_motif_class_card(info):
    ...
    return f"<div style='padding:0.25rem 0.5rem;background:{s['background']};border-radius:6px;border-left:3px solid {s['border']};'><div style='font-weight:600;color:{s['text']};font-size:0.85rem;line-height:1.3;'>{info['num']}. {info['name']}</div><div style='color:{s['text']};font-size:0.78rem;margin-top:0.05rem;line-height:1.2;'>{info['subtitle']}</div></div>"
...
def render():
    ...
    with col_science:
        st.markdown(f"<div style='background:{c['white']};padding:0.4rem 1rem;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.08);border:1px solid {c['neutral_200']};height:100%;'><h2 style='color:{c['text']};font-size:1.3rem;margin:0 0 0.4rem 0;font-weight:600;'>Scientific Foundation</h2><p style='color:{c['neutral_700']};font-size:1.0rem;line-height:1.6;margin-bottom:0.6rem;'><b style='color:{c['primary']};'>Non-canonical DNA structures</b> are critical regulatory elements implicated in genome stability, transcriptional regulation, replication, and disease mechanisms.</p><ul style='color:{c['neutral_700']};font-size:0.95rem;line-height:1.5;padding-left:1.5rem;margin-bottom:0.6rem;'><li><b>Genome Instability:</b> Hotspots for mutations and chromosomal rearrangements</li><li><b>Gene Regulation:</b> Promoter and enhancer activity modulation</li><li><b>DNA Replication:</b> Origins of replication and fork progression</li><li><b>Disease Association:</b> Cancer, neurological disorders, and aging</li></ul><div style='background:linear-gradient(135deg,{c['primary']} 0%,{c['secondary']} 100%);padding:0.3rem 0.8rem;border-radius:8px;text-align:center;box-shadow:0 4px 12px {c['shadow']};'><h3 style='color:{c['white']};margin:0 0 0.15rem 0;font-size:1.0rem;'>{UI_TEXT['home_call_to_action_title']}</h3><p style='color:rgba(255,255,255,0.95);margin:0 0 0.3rem 0;font-size:0.88rem;'>{UI_TEXT['home_call_to_action_text']}</p><div style='background:{c['white']};color:{c['primary']};padding:0.35rem 0.9rem;border-radius:6px;display:inline-block;font-weight:600;font-size:0.9rem;'>{UI_TEXT['home_call_to_action_button']}</div></div></div>", unsafe_allow_html=True)
    cards_html = ''.join(_build_motif_class_card(info) for info in MOTIF_CLASS_INFO)
    st.markdown(f"<div style='background:{c['white']};padding:0.4rem 1rem;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.08);border:1px solid {c['neutral_200']};margin-top:0.75rem;'><h2 style='color:{c['text']};font-size:1.3rem;margin:0 0 0.4rem 0;font-weight:600;'>Detected Motif Classes</h2><div style='display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:0.4rem;margin-top:0.4rem;'>{cards_html}</div></div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f"<div style='background:{c['white']};padding:0.4rem 1rem;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.08);border:1px solid {c['neutral_200']};margin-top:0.75rem;'><h2 style='color:{c['primary']};font-size:1.3rem;margin:0 0 0.4rem 0;font-weight:600;'>How to Cite</h2><div style='background:{c['neutral_50']};padding:0.3rem 0.8rem;border-radius:6px;border-left:4px solid {c['primary']};font-family:\"Courier New\",monospace;font-size:0.9rem;line-height:1.5;color:{c['neutral_700']};'><b>NonBFinder: Comprehensive Detection and Analysis of Non-B DNA Motifs</b><br>Dr. Venkata Rajesh Yella<br>GitHub: <a href=\"https://github.com/VRYella/NonBFinder\" style=\"color:{c['primary']};\">https://github.com/VRYella/NonBFinder</a><br>Email: yvrajesh_bt@kluniversity.in</div><p style='color:{c['neutral_600']};font-size:0.9rem;margin-top:0.4rem;line-height:1.5;'>If you use NonBFinder in your research, please cite this resource. For methodology references, see the <b>Documentation</b> tab.</p></div>", unsafe_allow_html=True)
