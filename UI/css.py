...
def load_css(theme_name=None):
    ...
    # Load scientific theme overrides (applied after styles.css for correct cascade)
    theme_css_path = os.path.join(os.path.dirname(__file__), "theme.css")
    try:
        with open(theme_css_path, "r") as f: theme_css_content = f.read()
    except: theme_css_content = ""
