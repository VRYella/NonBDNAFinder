...
DESCRIPTION:
    Consolidated core module containing all motif detection logic, detectors,
    and analysis orchestration. Merges functionality from nonbscanner.py,
    detectors.py, and scanner_agent.py into a single unified engine.
    
    Performance optimizations:
    - Compiled regex patterns with caching
    - Optimized overlap removal (O(m log m) instead of O(m²))
    - Parallel detector execution
    - Memory-efficient chunked processing

MAIN COMPONENTS:
    1. Detector Classes (from detectors.py):
       - BaseMotifDetector: Abstract base for all detectors
       - 9 specialized detectors for each motif class
       
    2. Scanner Class (from nonbscanner.py):
       - NonBScanner: Main analysis orchestrator
       - Progress tracking and callbacks
       
    3. Parallel Scanner (from scanner_agent.py):
       - ParallelScanner: Multi-core scanning agent
       - Hyperscan integration (optional)
...
USAGE:
    ...
    # Advanced API with progress
    scanner = NonBScanner()
    motifs = scanner.analyze_sequence(sequence, name, progress_callback=callback)
"""
...
from typing import List, Dict, Any, Tuple, Optional, Union, Callable, overload, Literal
...
# ==============================================================================
# DETECTOR CLASSES (from detectors.py)
# ==============================================================================
