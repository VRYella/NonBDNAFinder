...
except ImportError as e: HYPERSCAN_ERROR = f"Hyperscan not installed: {e}"; logger.info(f"Using pure Python fallback. {HYPERSCAN_ERROR}")
except Exception as e: HYPERSCAN_ERROR = f"Hyperscan init failed: {e}"; logger.warning(f"Using pure Python fallback. {HYPERSCAN_ERROR}")
...
for k in _THEME_DEFAULTS:
    if k not in st.session_state: st.session_state[k] = _THEME_DEFAULTS[k]
