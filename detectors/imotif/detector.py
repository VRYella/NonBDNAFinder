"""
i-Motif DNA Detector Module

This module provides detection and analysis of i-motif DNA structures, which are
quadruplex structures formed by cytosine-rich sequences under acidic conditions.

i-Motifs are four-stranded DNA structures stabilized by hemiprotonated cytosine-
cytosine+ (C:C+) base pairs. They form at pH < 7 and have been implicated in 
gene regulation, particularly at promoter regions.

Scientific Background:
---------------------
i-Motifs form from C-rich sequences arranged in C-tracts separated by loops. The
canonical i-motif consists of four C-tracts (C3+) connected by short linker regions.
Recent research has identified HUR AC-motifs as a variant class with alternating
adenine and cytosine-rich regions.

Key Research References:
-----------------------
- Gehring et al. (1993): First structural characterization of i-motif DNA
  Nature 363, 561-565. DOI: 10.1038/363561a0
  
- Leroy et al. (1995): Solution structures of i-motif tetramers
  Nucleic Acids Research 23(8), 1392-1399. DOI: 10.1093/nar/23.8.1392
  
- Benabou et al. (2014): Fundamental aspects of i-motif structures
  RSC Advances 4(51), 26956-26980. DOI: 10.1039/C4RA02129K
  
- Hur et al. (2021): HUR AC-motif discovery and characterization
  Nucleic Acids Research. DOI: 10.1093/nar/gkab441

Detection Methods:
-----------------
1. Pattern-based detection: Regular expressions for canonical i-motif patterns
2. HUR AC-motif detection: Specialized patterns for A3-C3 alternating motifs
3. Validated sequence matching: Known i-motif forming sequences from literature
4. Scoring: C-tract density and arrangement analysis
...
# Import pattern definitions with fallback
try:
    from motif_patterns import IMOTIF_PATTERNS
except ImportError:
    IMOTIF_PATTERNS = {}

# i-Motif validated sequences from literature
VALIDATED_SEQS = [
...
class IMotifDetector(BaseMotifDetector):
    """Detector for i-motif DNA: canonical C-rich structures and HUR AC-motifs. References: Gehring (1993), Hur (2021), Benabou (2014)."""
    ...
    def detect_motifs(self, sequence: str, sequence_name: str = "sequence") -> List[Dict[str, Any]]:
        """Detect i-motif structures with component details and overlap resolution. Ensures only longest/highest-scoring non-overlapping motif reported per subclass."""
        seq = sequence.upper()
        motifs = []
        ...
        subclass_map = {
            ...
            'hur_ac_motif': 'AC-motif',
            'ac_motif_hur': 'AC-motif'
        }
        ...
        for i, accepted in enumerate(accepted_motifs):
            ...
            subclass = subclass_map.get(class_name, 'Canonical i-motif')
            score = accepted.get('score', 0)
            
            # Normalize class/subclass using canonical taxonomy (subclasses are already canonical)
            ...
            gc_total = self._calc_gc(motif_seq)
            gc_stems = 0
            if c_tracts:
